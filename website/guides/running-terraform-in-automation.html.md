...
2. Terraform produces a plan for changing resources to match the current
configuration.
3. A human review that plan, to ensure it is acceptable.
4. Terraform takes the actions described in the plan.
...
configuration across environments. For example, the environments may exist
in entirely separate accounts within the target service, and thus need to
use different credentials or endpoints for the backend itself. In such
situations, backend configuration settings can be overridden via
...
With this directory populated, the usual auto-download and discovery behavior
can be bypassed using the `-plugin-dir` option to `terraform init`:
